...
const bool diff[3][3] = {{0,0,1},{0,0,1},{0,0,1},{1,1,0}};
const int M = 2005; 
int n,m,fanzhu,deadfan,rounds,tmps[M],used[M];
char ch,cu;
...
void get_cards(int cur){
	a[cur].cards[++a[cur].cnt] = cards_piles.front();
	if(cards_pile.size()>1) cards_pile.pop_front();
}

bool ought(int cur){
	int nxt = a[cur].nxt;
	if(a[nxt].perfo == 0) return 0;
	else if(a[nxt].perfo == 1) return diff[a[cur].iden][a[nxt].iden];
...
void respond_peach(int cur,int s,int t){
	int re = find(cur,'P');
	if(cur == user){
		re = 0;
		for(int i=1;i<=a[cur].cnt;i++) if(used[i]!=round && a[cur].cards[i] == 'P'){
			re = i;
			break;
		}
...
bool respond_kill(int cur,int user){
	...
	if(cur == user){
		re = 0;
		for(int i =1;i<a[cur].cnt;i++) if(used[i]!=rounds && a[cur].cards[i] == 'K'){
			re = i;
			break;
		}
...
void do_fight(int cur,int aim,int user){
	...
	for(;;){
		if(a[cur].iden == 0 && a[aim].iden == 1){
			lose_blood(aim,user);
			if(a[aim].bloods<1) deadfan+=(a[aim].iden==2),a[aim].dead=1,chang_link(aim);
			if(fanzhu == deadfan || a[1].dead) return;
		...
		if(!respond_kill(cur,user)){
			lose_blood(cur,user);
			if(a[cur].bloods<1) deadfan+=(a[cur].iden == 2),a[cur].dead=1,chang_link(cur);
			if(fanzhu == deadfan || a[1].dead) return;
			if(a[aim].bloods<1) pend(aim,cur);
		}
	}
}
